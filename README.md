...
As a firmware synthesizer, i used opensource Yosys/Apicula/NextPNR, because due to war sanctions, we here in Russia were cut off from many programs, including Gowin IDE free license program. UPD. Using Gowin IDE for synthesize, results were the same. <strike>It seems that prorerly working async FIFO is required.</strike> UPD2: It seems that the parallel data from DAC routed to FPGA is coming in corrupted way, and the reason is FPGA's pins default purposes. I must relocate some of DAC[15:0] bits to another pins.
